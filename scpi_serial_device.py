...
class scpi_serial_device(scpi_device):
    ...
    def connect(self, port_name = None):  # serial_connect, could also be called simply connect, and

        if(port_name != None):
            self.port_name = port_name

        logging.debug("scpi_serial_device.serial_connect() method called")
        logging.debug("Port Name:")
        logging.debug(self.port_name)
        logging.debug("port name " + self.port_name)
    ...
    def send_command(self,command):  # do I need another thread for this ???
        logging.debug("scpi_serial_device.send_command() method called")
    ...
    def receive_response(self):
        logging.debug("scpi_serial_device.receive_response() method called")
        try:
...
if __name__ == "__main__":
    ...
    dev.connect("COM9")
    dev.device_id = "GW INSTEK,AFG-2105"
    confirm = dev.confirm_device_id()
